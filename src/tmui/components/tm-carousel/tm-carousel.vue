    ...
    <view class="flex flex-col flex-col-center-center">
        ...
        >
            <swiper
                ...
                :style="[{ width: `${props.width}rpx`, height: `${props.height}rpx` }]"
            >
                <swiper-item v-for="(item, index) in _list" :key="index" :style="[{ width: `${props.width}rpx`, height: `${props.height}rpx` }]">
                    <tm-image v-if="item.type == listItemType.img" :src="item.url" :width="props.width" :height="props.height"></tm-image>
                    <tm-image
                        v-if="item.type == listItemType.video && item.img && _current != index"
                        :src="item.img"
                        :width="props.width"
                        :height="props.height"
                    ></tm-image>
                    <video
                        id="video"
                        v-if="item.type == listItemType.video && _current === index"
                        :src="item.url"
                        :style="[{ width: `${props.width}rpx`, height: `${props.height}rpx` }]"
            ...
            >
                <tm-sheet
                    :margin="[10, 10]"
                    :follow-theme="_current == index ? props.followTheme : false"
                    :padding="[0, 0]"
                    :round="10"
                    @click="dotClick(index)"
                    :color="_current == index ? props.color : 'white'"
